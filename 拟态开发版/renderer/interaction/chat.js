// ═══════════════════════════════════════════════════════════
//  Nitai v2.0 — Chat Interaction
//
//  Press Enter → text input dialog → mock chat response.
//  If backend /api/chat is available, uses it; else mock.
// ═══════════════════════════════════════════════════════════
...
;(function () {
  ...
  const MOCK_REPLIES = [
    ...
    { trigger: ['召唤', '过来', 'come'], replies: ['点右键菜单选"召唤到这里"就可以啦~ 🔮', '你想让我去哪？右键菜单 → 召唤！'] },
  ...
  function getMockReply(text) {
    ...
    for (const entry of MOCK_REPLIES) {
      ...
      for (const t of entry.trigger) {
        if (lower.includes(t)) return entry.replies[Math.floor(Math.random() * entry.replies.length)];
      }
    }
  ...
  function showChatInput() {
    ...
    dialog.style.cssText = 'background:white;border-radius:16px;padding:20px 24px;box-shadow:0 8px 32px rgba(0,0,0,0.2);text-align:center;font-family:"Microsoft YaHei",sans-serif;min-width:280px;';
    ...
    function submit() {
      ...
      // Wander commands
      if (/闲逛|走走|散步|wander/i.test(text)) {
        if (M.AI) M.AI.toggleWander();
        return;
      }
      if (/过来|到这里|summon|召唤/i.test(text)) {
      ...
      if (/听歌|music|耳机|来点音乐/i.test(text)) {
        ...
        if (M.Bubble) M.Bubble.show('🎧 正在听歌… ♪', { duration: 2500 });
        return;
      }
      if (/停止|stop|别唱了/i.test(text)) {
        M.Anim.showHeadphones = false;
        M.FSM.transitionTo('idle');
        if (M.Bubble) M.Bubble.show('好吧~ (´・ω・`)', { duration: 1500 });
        return;
      }

      // Wander toggle
      if (/别逛了|停下|休息/i.test(text)) {
        M.wanderEnabled = false;
        if (M.Bubble) M.Bubble.show('好，休息一会 💤', { duration: 1500 });
        return;
      }

      // Thought bubble
      if (M.Bubble) M.Bubble.show('" ' + text + ' "', { duration: 800, thought: true });
  ...
  function trySendToBackend(userText, fallbackReply) {
    if (!M.config.apiEnabled) { showReply(fallbackReply); return; }
    ...
    try {
      ...
      const req = M.http.request(options, (res) => {
        ...
        res.on('end', () => {
          try { const data = JSON.parse(body); showReply(data.reply || data.response || fallbackReply); }
          catch (e) { showReply(fallbackReply); }
        });
      });
      req.on('error', () => { showReply(fallbackReply); });
  ...
  function showReply(text) {
    M.FSM.transitionTo('happy');
    if (M.Bubble) M.Bubble.show(text, { duration: Math.max(2000, text.length * 80) });
    setTimeout(() => {
  ...
  function setupChat() {
    ...
    console.log('[chat] v2.0 — Enter-key chat + wander/summon commands');
  }
