# PROJECT_STATUS.md — 拟态 (Nitai) Desktop Pet

> 最后更新：2026-07-02 | 版本：**v1.0.0** — 独立后首个正式版本（含架构重构）
>
> v1.0.0 从晓风主项目分离后，进行了完整的架构重构：从静态窗口到移动角色的范式转换。

## 核心范式转换

**重构前**: 角色禁锢在一个静态窗口里，窗口位置固定。
**v1.0.0**: 角色拥有**世界坐标**（屏幕坐标），在整个桌面上自由移动。窗口跟随角色。

```
重构前:  Window → Character inside (static)
...
### v1.0.0 (2026-07-02) — 独立后首个正式版本 + 架构重构 🔥
